-- ====================================================================
--  SUPABASE DATABASE SETUP FOR CHATIZY (E2EE CHAT APP)
--  Run this script in the Supabase SQL Editor to initialize all tables,
--  indexes, triggers, functions, and RLS (Row Level Security) policies.
-- ====================================================================
...
-- Auto-create/update profile trigger on new signup
...
-- ── 3. Create One-Time Prekeys Table ──────────────────────────────────
...
-- ── 5. Create Conversation Members Table ─────────────────────────────
...
-- ── 6. Create Device Envelopes Table ─────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_de_target_pending ON public.device_envelopes(target_device_id, created_at) WHERE status = 'pending';
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_de_idempotent ON public.device_envelopes(client_message_id, target_device_id);

-- ── 7. Create Media Objects Table ────────────────────────────────────
...
-- ── 8. Create Device Sync Cursors Table ──────────────────────────────
...
-- ── 9. Create Linked Device Sessions Table ──────────────────────────
...
-- ── 10. Create Delivery Events Table ─────────────────────────────────
...
-- ── 11. Enable RLS and Configure Row Level Security Policies ────────
...
CREATE POLICY "devices_select_authenticated" ON public.devices FOR SELECT USING (auth.uid() IS NOT NULL);
...
-- one_time_prekeys policies
DROP POLICY IF EXISTS "otpk_select_authenticated" ON public.one_time_prekeys;
CREATE POLICY "otpk_select_authenticated" ON public.one_time_prekeys FOR SELECT USING (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "otpk_insert_own_device" ON public.one_time_prekeys;
CREATE POLICY "otpk_insert_own_device" ON public.one_time_prekeys FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = one_time_prekeys.device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "otpk_update_own_device" ON public.one_time_prekeys;
CREATE POLICY "otpk_update_own_device" ON public.one_time_prekeys FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = one_time_prekeys.device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "otpk_delete_own_device" ON public.one_time_prekeys;
CREATE POLICY "otpk_delete_own_device" ON public.one_time_prekeys FOR DELETE USING (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = one_time_prekeys.device_id AND devices.user_id = auth.uid())
);
...
CREATE POLICY "conversations_select_member" ON public.conversations FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.conversation_members WHERE conversation_members.conversation_id = conversations.id AND conversation_members.user_id = auth.uid() AND conversation_members.left_at IS NULL)
);

DROP POLICY IF EXISTS "conversations_insert_authenticated" ON public.conversations;
CREATE POLICY "conversations_insert_authenticated" ON public.conversations FOR INSERT WITH CHECK (auth.uid() = created_by);

-- conversation_members policies
DROP POLICY IF EXISTS "cm_select_member" ON public.conversation_members;
CREATE POLICY "cm_select_member" ON public.conversation_members FOR SELECT USING (
...
CREATE POLICY "cm_insert_authenticated" ON public.conversation_members FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "cm_update_own" ON public.conversation_members;
CREATE POLICY "cm_update_own" ON public.conversation_members FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- device_envelopes policies
DROP POLICY IF EXISTS "de_select_target" ON public.device_envelopes;
CREATE POLICY "de_select_target" ON public.device_envelopes FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = device_envelopes.target_device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "de_select_sender" ON public.device_envelopes;
CREATE POLICY "de_select_sender" ON public.device_envelopes FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = device_envelopes.sender_device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "de_insert_sender" ON public.device_envelopes;
CREATE POLICY "de_insert_sender" ON public.device_envelopes FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = device_envelopes.sender_device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "de_update_target" ON public.device_envelopes;
CREATE POLICY "de_update_target" ON public.device_envelopes FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = device_envelopes.target_device_id AND devices.user_id = auth.uid())
);

-- media_objects policies
DROP POLICY IF EXISTS "mo_select_member" ON public.media_objects;
CREATE POLICY "mo_select_member" ON public.media_objects FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.conversation_members WHERE conversation_members.conversation_id = media_objects.conversation_id AND conversation_members.user_id = auth.uid() AND conversation_members.left_at IS NULL)
);

DROP POLICY IF EXISTS "mo_insert_authenticated" ON public.media_objects;
CREATE POLICY "mo_insert_authenticated" ON public.media_objects FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = media_objects.uploader_device_id AND devices.user_id = auth.uid())
);

-- device_sync_cursors policies
DROP POLICY IF EXISTS "dsc_select_own" ON public.device_sync_cursors;
CREATE POLICY "dsc_select_own" ON public.device_sync_cursors FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = device_sync_cursors.device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "dsc_insert_own" ON public.device_sync_cursors;
CREATE POLICY "dsc_insert_own" ON public.device_sync_cursors FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = device_sync_cursors.device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "dsc_update_own" ON public.device_sync_cursors;
CREATE POLICY "dsc_update_own" ON public.device_sync_cursors FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = device_sync_cursors.device_id AND devices.user_id = auth.uid())
);

-- linked_device_sessions policies
DROP POLICY IF EXISTS "lds_select_owner" ON public.linked_device_sessions;
CREATE POLICY "lds_select_owner" ON public.linked_device_sessions FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.devices WHERE (devices.id = linked_device_sessions.primary_device_id OR devices.id = linked_device_sessions.linked_device_id) AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "lds_insert_primary" ON public.linked_device_sessions;
CREATE POLICY "lds_insert_primary" ON public.linked_device_sessions FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = linked_device_sessions.primary_device_id AND devices.user_id = auth.uid())
);

DROP POLICY IF EXISTS "lds_update_owner" ON public.linked_device_sessions;
CREATE POLICY "lds_update_owner" ON public.linked_device_sessions FOR UPDATE USING (
  EXISTS (SELECT 1 FROM public.devices WHERE (devices.id = linked_device_sessions.primary_device_id OR devices.id = linked_device_sessions.linked_device_id) AND devices.user_id = auth.uid())
);

-- delivery_events policies
DROP POLICY IF EXISTS "devt_select_participant" ON public.delivery_events;
CREATE POLICY "devt_select_participant" ON public.delivery_events FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.device_envelopes de JOIN public.devices d ON (d.id = de.sender_device_id OR d.id = de.target_device_id) WHERE de.id = delivery_events.envelope_id AND d.user_id = auth.uid())
);

DROP POLICY IF EXISTS "devt_insert_target" ON public.delivery_events;
CREATE POLICY "devt_insert_target" ON public.delivery_events FOR INSERT WITH CHECK (
  EXISTS (SELECT 1 FROM public.devices WHERE devices.id = delivery_events.device_id AND devices.user_id = auth.uid())
);
